...
{
    /// <summary>
    /// 保険。このショットが0点だった場合（壁命中・枠外・時間切れ）、代わりに保険点を獲得する（MEDALS.md 4章#11）。
    /// 属性判定で打ち消された場合はそもそも呼ばれないため、無条件の保険にはならない（MEDALS.md 4章補足）。
    /// </summary>
    public sealed class SafetyNetEffect : MedalEffectBase
    {
